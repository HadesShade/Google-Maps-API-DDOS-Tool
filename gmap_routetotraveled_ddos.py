...
def generate_url(apikey):
    ...
    return f"https://roads.googleapis.com/v1/snapToRoads?interpolate=true&path={lat_a}.{lat_b},{lng_a}.{lng_b}&key={apikey}"

def do_attack():
    apikey = str(input("Enter Google Maps API Key that vulnerable to route to traveled (snap to roads): "))

...
def get_searchterm():
    ...
    return f"departure_time=now&destinations={city_line1.strip()}&origins={city_line2.strip()}"

def generate_url(apikey, term):
    return f"https://maps.googleapis.com/maps/api/distancematrix/json?{term}&key={apikey}"

def do_attack():
    apikey = str(input("Enter Google Maps API Key that vulnerable to advanced distance matrix: "))
